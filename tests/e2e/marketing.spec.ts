...
test('home page publishes identity, navigation, metadata, and security headers', async ({
  ...
  await expect(
    page.locator(
      'nav[aria-label="Primary navigation"] a[href="/#products"]',
    ),
  ...
  await expect(
    page.getByRole('link', { name: 'Engineering Standards v1' }),
  ).toBeVisible();
...
test(
  'removed marketing routes redirect to landing-page sections',
  async ({ request }) => {
    for (const route of [
